...
io.on("connection", function(socket) {
    ...
    socket.on('join-room',(target)=> {  // challenger join room
        ...
        RoomsList[`${target}-${socket.Username}`] = {
            challenger : socket.Username,
            challengerID : mangUser[`${socket.Username}`],
            target : target,
            targetID : mangUser[`${target}`]
        }
    ...
    // socket.on('picked', function (data,fn) {
    //     io.sockets.in(`${socket.myGame}`).emit('pickedtoRoom', data,(resData)=>{
    //         fn(resData)
    //     }); // tu socket chuyen data vao room
    // });
    // socket.on('moved', function (data,fn) {
    //      // tu socket chuyen data vao room
    //     io.sockets.in(`${socket.myGame}`).emit('movedtoRoom', data,(resData)=> {
    //         fn(resData)
    //     });
    // });
